...
describe('v2 真实链路：分片 → 组装 → importFullData', () => {
    ...
    it('聊天装扮随备份走：角色 chatFineTune 与主题微调字段 v2 往返不丢', async () => {
        ...
        const char = {
            ...
            preReplyMcpRules: [{ id: 'usage', name: '检查手机', mode: 'always', serverId: 'mcp1', toolName: 'query_events', argumentsJson: '{"hours":6}', promptTemplate: '{{result}}', maxResultChars: 8000, minIntervalMinutes: 0, onFailure: 'continue' }],
